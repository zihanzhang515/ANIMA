...
class StudyManager:
    ...
    def _start_s5_logging(self):
        """Log S5 (input rate) every 5 seconds during an active phase."""
        def _loop():
            while True:
    ...
    def toggle_pause(self):
        ...
        if self.is_paused:
            print("\n" + "="*40)
            print("  [STUDY] SYSTEM PAUSED")
            print("  (Sensors/Replay ignored. Press 'p' to resume)")
            print("="*40 + "\n")
        else:
            print("\n" + "="*40)
            print("  [STUDY] SYSTEM RESUMED")
            print("="*40 + "\n")
    ...
    def record_emotion(self, emotion_name):
        ...
        # Only record within the 20-minute window
        if elapsed > 20 * 60:
    ...
    def print_analysis(self, phase_name):
        ...
        # For each emotion trigger, find the first S5 snapshot that occurs after it
        for e in self.recordings:
            trigger_t = e['time_offset']
            emo = e['emotion'].upper()

            target_snap = None
            for snap in self.s5_log:
